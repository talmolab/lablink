...
Publish the client package to PyPI using the automated workflow.
...
git push origin lablink-client-service_v0.0.7a0
...
**Format**: `lablink-client-service_v{version}`

**Examples**:
- `lablink-client-service_v0.0.7a0` (alpha)
- `lablink-client-service_v0.1.0` (stable)
...
cat packages/client/pyproject.toml | grep "version ="
...
pip index versions lablink-client
...
pip install lablink-client==0.0.7a0

# Test entry points
subscribe --help
check_gpu --help
update_inuse_status --help
```
...
```bash
# Delete local tag
git tag -d lablink-client-service_v0.0.7a0

# Delete remote tag
git push origin --delete lablink-client-service_v0.0.7a0

# Recreate with correct version
git tag lablink-client-service_v0.0.8a0
git push origin lablink-client-service_v0.0.8a0
```
...
**Solutions**:
1. Check tag: `lablink-client-service_v0.0.7a0`
2. Check `pyproject.toml`: `version = "0.0.7a0"`
...
- `/publish-allocator` - Publish allocator package
