...
@patch("lablink_allocator_service.utils.aws_utils.boto3.client")
def test_ssm_run_command_success(mock_boto_client):
    """Test successful SSM RunCommand."""
    mock_ssm = MagicMock()
    ...
    result = aws_utils.ssm_run_command(
        "i-abc123",
    ...
    mock_ssm.send_command.assert_called_once_with(
        ...
        Parameters={"commands": ["cloud-init clean", "reboot"]},
        TimeoutSeconds=60,
    )


@patch("lablink_allocator_service.utils.aws_utils.boto3.client")
def test_ssm_run_command_failure(mock_boto_client):
    """Test SSM RunCommand failure (e.g., SSM agent not running)."""
    from botocore.exceptions import ClientError
