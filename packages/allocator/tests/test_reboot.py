...
def test_record_heartbeat_updates_columns(db_instance):
    """record_heartbeat persists all reported columns and bumps last_seen_at."""
    ...
    # v2 dropped crd_active from the payload.
...
def test_record_reboot(db_instance):
    ...
    # The student's assignment (useremail) is preserved across reboots.
    # v2 has no per-session columns to clear here.
    query = db_instance.cursor.execute.call_args[0][0]
